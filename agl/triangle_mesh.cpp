...
void TriangleMesh::setIsDynamic(bool on) {
  assert(_initialized == false);
  _isDynamic = on;
}

void TriangleMesh::setVertexData(VertexAttribute type,
...
vec4 TriangleMesh::vertexData(VertexAttribute type, int vertexId) const {
  assert(vertexId >= 0 && vertexId < _nVerts);
