...
void Renderer::initBillboards()
{
   ...
   glBindBuffer(GL_ARRAY_BUFFER, mBBVboPosId); // always bind before setting data
   glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (GLubyte*)NULL);

   mBBShaderId = loadShader("../shaders/billboard.vs", "../shaders/billboard.fs");
}
...
void Renderer::begin(GLuint texIf, BlendMode mode)
{
   ...
   glUniformMatrix4fv(glGetUniformLocation(mBBShaderId, "uVP"), 1, GL_FALSE, &mvp[0][0]);
   glUniform3f(glGetUniformLocation(mBBShaderId, "uCameraPos"), mLookfrom[0], mLookfrom[1], mLookfrom[2]);
...
{
   assert(mInitialized);
   glUniform3f(glGetUniformLocation(mBBShaderId, "uOffset"), pos[0], pos[1], pos[2]);
   glUniform4f(glGetUniformLocation(mBBShaderId, "uColor"), color[0], color[1], color[2], color[3]);
...
void Renderer::skybox()
{
   ...
   mat4 mvp = mProjectionMatrix * mViewMatrix;
   glUniformMatrix4fv(glGetUniformLocation(mCMShaderId, "uVP"), 1, GL_FALSE, &mvp[0][0]);
   
   mSkybox->render(); 
}

void Renderer::mesh(const mat4& trs, const Mesh& mesh)
{
   assert(mInitialized);
   blendMode(DEFAULT);
   ...
   glUniform4f(glGetUniformLocation(mMShaderId, "uLight.position"), 100.0, 100.0, 100.0, 1.0);
   glUniform3f(glGetUniformLocation(mMShaderId, "uLight.color"), 1.0, 1.0, 1.0);

   mat4 mv = mViewMatrix * trs * mesh.translateAndScale();
